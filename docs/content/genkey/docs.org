...
* Basic Usage
  ...
  =genkey=, so if you're using Windows, make sure to correct it. Also
...
* Layouts
  Layouts are stored as basic text files in the =layouts=
  ...
  file is *not* used by the program. Note that if two files have the
  ...
  would expect. Thumb keys are not supported in genkey, but [[https://github.com/apsu/genkey][Apsu's
  fork]] does add experimental support.

** _generate
   The =_generate= layout file is used to define the fingermap for
   ...
   keys are labeled with =*=, and pinned keys are labeled with =X=.

* Weighting
  The weights that determine how layouts are scored is configured in
  =weights.hjson=. Though the base weights are made to be an okay
...
** Finger Speed
   ...
   [[*Distance]]). $n$ is the 'length' of the skip-gram, or how the
   distance between characters in the sequence. For a bigram, $n$
   is 1. For a skip-1-gram, (e.g. =t_e= in =the=) $n$ is 2.
   ...
   genkey allows you to weight it.[fn:1]
...
*** Redirects
    ...
    =sfd= would be a redirect on QWERTY, because the direction of =sf=
    is inward, but the direction of =fd= is outward. Since redirects
    are generally undesired, they add to the score. 
*** Onehands
    Onehand rolls are defined as trigrams where all keys are on the
    same, and they all follow the same direction. For example, =sdf=
...
** Load
   ...
   updates =data.json= according to the text data that it
